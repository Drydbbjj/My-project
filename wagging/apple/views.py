...
class Login(View):
    def get(self, request):
        logger.info('Login GET request received')
        if not request.user.is_authenticated:
            fm = LoginForm()
...
def get_following_persons(person_id):
    logger.info('Getting following persons for person ID %s', person_id)
...
def getUserLikeStatus(person_id, post_id):
    logger.info('Getting like status for person ID %s and post ID %s', person_id, post_id)
...
def getCommentCount(postid):
    logger.info('Getting comment count for post ID %s', postid)
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            try:
                ...
                messages.error(request, "Person object does not exist for this user")
...
def pic_update(request):
    ...
    print('pic update')
    return redirect(f'/profile/{personid}')
